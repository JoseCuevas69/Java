...
/*
    ...
    */
    ...
    DisplayAlerta(parTipo){
      ...
      console.log('DisplayAlerta', parTipo);
      const Toast = swal.mixin({
        toast: true,
        position: 'top-end',
        showConfirmButton: false,
    ...
    getEstatusCondicion(parIdRegla: number, parIdCondicion: number, parModulo: string = ''): boolean{
      const dataSaved = JSON.parse(sessionStorage.getItem('ReglasCondiciones'));
      console.log(dataSaved , 'comprobar si esta el condicion ');
      let EstatusCondicion;
      for (const element of dataSaved) {
        if (parModulo !== '') {
          if (element.idRegla === parIdRegla && element.idCondicion === parIdCondicion && element.nomenclatura === parModulo) {
            if (element.estatusCondicion === 1){
              EstatusCondicion = true;
            } else {
              EstatusCondicion = false;
            }
          }
        }
      }
      console.log(EstatusCondicion , 'estatus condicion');
      return EstatusCondicion;
    }
    ////////////////////////////////////////////////////////////////////////////
    //#region Metodos de Consultas a BD
    ////////////////////////////////////////////////////////////////////////////
