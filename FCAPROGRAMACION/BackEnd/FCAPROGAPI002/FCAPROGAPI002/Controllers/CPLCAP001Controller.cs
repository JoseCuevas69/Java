...
namespace FCAPROGAPI002.Controllers
{
    ...
    [Route("/[controller]")]
    [ApiController]
    public class CPLCAP001Controller : Controller
    {
        private readonly TokenData datosToken = new TokenData();

        public CPLCAP001Controller(IOptions<AppSettings> AppSettings, IHttpContextAccessor httpContext)
        {
        ...
        public async Task<IActionResult> cargaInfo02()
        {
            try
            {
                return Ok(await new CPLCAP001Business().cargaInfo02(datosToken));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> registrar(ListaDataAnchosCPLDAT003 Datos)
        {
            try
            {
                return Ok(await new CPLCAP001Business().registrar(datosToken, Datos));
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, $"Error, {ex.Message}");
