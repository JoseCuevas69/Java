...
BEGIN
	...
	IF @Justificacion = 0 BEGIN
		 IF @UltimoP			    = 1 BEGIN SET @UltimoPFiltro = ' AND a.[ultimo proceso]= 1 '  END
		  ...
		  IF @Justificacion = 1
			BEGIN
				...
				SET @JustificacionJoin = 'JOIN Cajas01..CmoDat231 d      LEFT JOIN Capacitacion01..Empleados e ON e.Clave = d.UsuarioERP       LEFT JOIN Capacitacion02..Empleados f  ON f.Clave = d.UsuarioERP   ON d.Programa = a.Programa AND d.Estatus = 0    LEFT join gxCajas01..FCPG011CT g on d.FCPRJUCAPRId = g.FCPRJUCAPRId '
		...
		from Cajas01..cmodat020 a 
			...
			JOIN Cajas01..CmoTjCat004 c ON b.[clave articulo] = c.[clave articulo] 
